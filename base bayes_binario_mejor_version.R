...
setwd("/Users/martinacazzolli/UBA2022")  #Establezco el Working Directory
...
data.frame(table(dataset$clase_ternaria,colum=c(dataset$cliente_vip)))
...
#datase$saldo_favor <- 
dataset$cdescubierto_preacordado_ctrx_quarter <- dataset$cdescubierto_preacordado/dataset$ctrx_quarter

dataset$active_quarter_ctrx_quarter <- dataset$active_quarter/dataset$ctrx_quarter
...
dir.create( "./exp/HT4110a/", showWarnings = FALSE )
setwd("./exp/HT4110a/")   #Establezco el Working Directory DEL EXPERIMENTO

#defino los archivos donde guardo los resultados de la Bayesian Optimization
archivo_log  <- "HT4110a.txt"
archivo_BO   <- "HT4110a.RDATA"
